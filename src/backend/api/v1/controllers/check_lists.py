...
def get_check_lists(skip: int = 0, limit: int = 50, db: async_session_maker = Depends(get_async_session)):
    return check_list_service.get_check_lists(db, skip, limit)


@check_lists_router.get("/{id}", response_model=CheckList)
def get_one_check_list(id: int, db: async_session_maker = Depends(get_async_session)):
...
def delete_check_list(id: int, db: async_session_maker = Depends(get_async_session)):
