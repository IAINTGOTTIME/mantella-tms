...
def get_test_suite(skip: int = 0, limit: int = 50, db=Depends(get_db)):
    return test_suite_service.get_test_suite(db, skip, limit)


@test_suite_router.get("/{id}", response_model=TestSuite)
def get_one_test_suite(id: int, db=Depends(get_db)):
...
def delete_test_suite(id: int, db: Session = Depends(get_db)):
