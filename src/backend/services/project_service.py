...
from entities.project_entities import RoleEnum, FunctionEnum
...
def update_project(db: Session,
    ...
    if user_id:
        ...
        if not new_user:
            raise HTTPException(detail=f"User wint id {user_id} not found",
                                status_code=404)
        if not role:
            raise HTTPException(detail=f"Select the user role",
                                status_code=400)
        if not function:
            raise HTTPException(detail=f"Select the function",
                                status_code=400)
        if function.add:
            if role.editor:
        ...
        if function.delete:
            if role.editor:
                if new_user in found.editors:
                    found.editors.append(user_id)
                raise HTTPException(detail=f"The user with the id {user_id} not found in the list of editors",
                                    status_code=409)
            if role.viewer:
                if new_user in found.viewers:
                    found.viewers.append(user_id)
                raise HTTPException(detail=f"The user with the id {user_id} not found in the list of editors",
                                    status_code=409)
    if not new_project:
